...
func_image_paths = {
    "history": "C:/Users/16324/Desktop/ISHW-Demo/images/func-history.jpg",
    "categories": "C:/Users/16324/Desktop/ISHW-Demo/images/func-categories.jpg",
    "book_search": "C:/Users/16324/Desktop/ISHW-Demo/images/func-booksearch.jpg",
    "dialog": "C:/Users/16324/Desktop/ISHW-Demo/images/func-dialog.jpg",
    "story": "C:/Users/16324/Desktop/ISHW-Demo/images/func-story.jpg",
    "fav_novel": "C:/Users/16324/Desktop/ISHW-Demo/images/func-favnovel.jpg",
    "writing": "C:/Users/16324/Desktop/ISHW-Demo/images/func-writing.jpg"
}
fav_novel_covers = [
    "photo/wbsxs.jpg",  # 我不是戏神
    "photo/srzy.webp",  # 十日终焉
    "photo/gmzz.jpg",  # 诡秘之主
    "photo/smfs.jpg",  # 神秘复苏
    "photo/dgyxOIP-C.webp",  # 道诡异仙
    "photo/zsyx.webp",  # 诸神愚戏
    "photo/dpcq.webp",  # 斗破苍穹
    "photo/kjdtmdl.jpg",  # 开局地摊卖大力
    "photo/ydmms.webp",  # 夜的命名术
    "photo/zt.webp", # 遮天
    "photo/xzhdx.jpg", # 雪中悍刀行
    "photo/dmbj.webp", # 盗墓笔记
    "photo/wzjsbyxzs.jpg", # 我在精神病院学斩神
    "photo/wyzslmddjlhhlb.jpg"  # 我一只史莱姆吊打巨龙很合理吧
]
...
/* 隐藏按钮：用于图片点击触发跳转（兼容Streamlit所有版本） */
.hidden-btn {
    visibility: hidden;
    position: absolute;
    width: 0;
...
# 渲染首页（核心修改：按钮→图片跳转）
def render_home():
    st.markdown('<h2 class="page-title">首页</h2>', unsafe_allow_html=True)
    ...
    api_key = st.text_input(
        ...
        placeholder="在此输入您的DeepSeek API密钥(如果你没有，可以用我的密钥：sk-2a83ebece503432b9eed4becf2478b24)"
    )
    ...
    # 核心功能区：HTML图片+自定义隐藏按钮（彻底解决className错误）
    st.markdown('<h3>核心功能</h3>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        # 1. HTML图片标签（支持样式和点击）
        st.markdown(f"""
        <img src="{func_image_paths['history']}" 
             style="width:100%; border-radius:6px; cursor:pointer;" 
             alt="1. 中国网文的发展历史" 
             class="img-btn"
             id="img-history">
        <p style="text-align:center; margin-top:8px; margin-bottom:0;">1. 中国网文的发展历史</p>
        """, unsafe_allow_html=True)
        # 2. 自定义隐藏按钮（用HTML按钮替代st.button，无className）
        st.markdown("""
        <button id="btn-history" 
                style="visibility:hidden; position:absolute; width:0; height:0; border:none; background:transparent;"
                onclick="window.streamlitSendMessage('history_click', '')">
        </button>
        """, unsafe_allow_html=True)
        # 3. JS：点击图片触发隐藏按钮
        st.markdown("""
        <script>
        // 图片点击事件
        document.getElementById('img-history').onclick = function() {
            document.getElementById('btn-history').click();
        }
        // Streamlit接收消息（触发页面跳转）
        const streamlitSendMessage = (type, data) => {
        ...
        """, unsafe_allow_html=True)
        # 4. Streamlit监听消息，执行跳转
        if st.session_state.get('btn_click_type') == 'history_click':
            go_to_history()
            # 重置状态，避免重复触发
            st.session_state['btn_click_type'] = None
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col2:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown(f"""
        <img src="{func_image_paths['categories']}" 
             style="width:100%; border-radius:6px; cursor:pointer;" 
             alt="2. 中国网文类型以及各类型书目" 
             class="img-btn"
             id="img-categories">
        <p style="text-align:center; margin-top:8px; margin-bottom:0;">2. 中国网文类型以及各类型书目</p>
        """, unsafe_allow_html=True)
        st.markdown("""
        <button id="btn-categories" 
                style="visibility:hidden; position:absolute; width:0; height:0; border:none; background:transparent;"
                onclick="window.streamlitSendMessage('categories_click', '')">
        </button>
        """, unsafe_allow_html=True)
        st.markdown("""
        <script>
        document.getElementById('img-categories').onclick = function() {
            document.getElementById('btn-categories').click();
        }
        </script>
        """, unsafe_allow_html=True)
        if st.session_state.get('btn_click_type') == 'categories_click':
            go_to_categories()
            st.session_state['btn_click_type'] = None
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col1:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown(f"""
        <img src="{func_image_paths['book_search']}" 
             style="width:100%; border-radius:6px; cursor:pointer;" 
             alt="3. 书荒查询" 
             class="img-btn"
             id="img-booksearch">
        <p style="text-align:center; margin-top:8px; margin-bottom:0;">3. 书荒查询</p>
        """, unsafe_allow_html=True)
        st.markdown("""
        <button id="btn-booksearch" 
                style="visibility:hidden; position:absolute; width:0; height:0; border:none; background:transparent;"
                onclick="window.streamlitSendMessage('booksearch_click', '')">
        </button>
        """, unsafe_allow_html=True)
        st.markdown("""
        <script>
        document.getElementById('img-booksearch').onclick = function() {
            document.getElementById('btn-booksearch').click();
        }
        </script>
        """, unsafe_allow_html=True)
        if st.session_state.get('btn_click_type') == 'booksearch_click':
            go_to_book_search()
            st.session_state['btn_click_type'] = None
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col2:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown(f"""
        <img src="{func_image_paths['dialog']}" 
             style="width:100%; border-radius:6px; cursor:pointer;" 
             alt="4. 与角色对话" 
             class="img-btn"
             id="img-dialog">
        <p style="text-align:center; margin-top:8px; margin-bottom:0;">4. 与角色对话</p>
        """, unsafe_allow_html=True)
        st.markdown("""
        <button id="btn-dialog" 
                style="visibility:hidden; position:absolute; width:0; height:0; border:none; background:transparent;"
                onclick="window.streamlitSendMessage('dialog_click', '')">
        </button>
        """, unsafe_allow_html=True)
        st.markdown("""
        <script>
        document.getElementById('img-dialog').onclick = function() {
            document.getElementById('btn-dialog').click();
        }
        </script>
        """, unsafe_allow_html=True)
        if st.session_state.get('btn_click_type') == 'dialog_click':
    ...
    with col1:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown(f"""
        <img src="{func_image_paths['story']}" 
             style="width:100%; border-radius:6px; cursor:pointer;" 
             alt="5. 穿书play" 
             class="img-btn"
             id="img-story">
        <p style="text-align:center; margin-top:8px; margin-bottom:0;">5. 穿书play</p>
        """, unsafe_allow_html=True)
        st.markdown("""
        <button id="btn-story" 
                style="visibility:hidden; position:absolute; width:0; height:0; border:none; background:transparent;"
                onclick="window.streamlitSendMessage('story_click', '')">
        </button>
        """, unsafe_allow_html=True)
        st.markdown("""
        <script>
        document.getElementById('img-story').onclick = function() {
            document.getElementById('btn-story').click();
        }
        </script>
        """, unsafe_allow_html=True)
        if st.session_state.get('btn_click_type') == 'story_click':
            go_to_story_mode()
            st.session_state['btn_click_type'] = None
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col2:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown(f"""
        <img src="{func_image_paths['fav_novel']}" 
             style="width:100%; border-radius:6px; cursor:pointer;" 
             alt="6. 开发者喜欢的小说" 
             class="img-btn"
             id="img-favnovel">
        <p style="text-align:center; margin-top:8px; margin-bottom:0;">6. 开发者喜欢的小说</p>
        """, unsafe_allow_html=True)
        st.markdown("""
        <button id="btn-favnovel" 
                style="visibility:hidden; position:absolute; width:0; height:0; border:none; background:transparent;"
                onclick="window.streamlitSendMessage('favnovel_click', '')">
        </button>
        """, unsafe_allow_html=True)
        st.markdown("""
        <script>
        document.getElementById('img-favnovel').onclick = function() {
            document.getElementById('btn-favnovel').click();
        }
        </script>
        """, unsafe_allow_html=True)
        if st.session_state.get('btn_click_type') == 'favnovel_click':
            go_to_my_love_novel()
            st.session_state['btn_click_type'] = None
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col1:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown(f"""
        <img src="{func_image_paths['writing']}" 
             style="width:100%; border-radius:6px; cursor:pointer;" 
             alt="7. AI带你写小说" 
             class="img-btn"
             id="img-writing">
        <p style="text-align:center; margin-top:8px; margin-bottom:0;">7. AI带你写小说</p>
        """, unsafe_allow_html=True)
        st.markdown("""
        <button id="btn-writing" 
                style="visibility:hidden; position:absolute; width:0; height:0; border:none; background:transparent;"
                onclick="window.streamlitSendMessage('writing_click', '')">
        </button>
        """, unsafe_allow_html=True)
        st.markdown("""
        <script>
        document.getElementById('img-writing').onclick = function() {
            document.getElementById('btn-writing').click();
        }
        </script>
        """, unsafe_allow_html=True)
        if st.session_state.get('btn_click_type') == 'writing_click':
            go_to_wrting()
            st.session_state['btn_click_type'] = None
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    st.info("开发者有话说！：作为一个资深的小说迷，我看过很多的网文，其中最喜欢的有番茄上的《我不是戏神》《斩神》《十日终焉》《诸神愚戏》《开局地毯卖大力》《我一只史莱姆吊打巨龙很合理吧》......以及几乎所有经典的玄幻小说，和克苏鲁风格，灵异风格的小说，比如诡秘之主，道诡异仙，神秘复苏之类的。如果有同道中人，欢迎加我QQ：1632444272！！！")
...
# 初始化会话状态（需确保在所有页面渲染前执行，若已有可忽略）
if 'btn_click_type' not in st.session_state:
    st.session_state['btn_click_type'] = None

#################################################################################################################################################################################################################################
# 渲染历史页面（仅添加样式，逻辑不变）
def render_history():
    st.markdown('<h2 class="page-title">中国网文的发展历史</h2>', unsafe_allow_html=True)
    st.button("返回首页", on_click=go_to_home, className="nav-btn")
    # 历史文本添加卡片样式
    st.markdown('<div class="card">', unsafe_allow_html=True)
...
def render_categories():
    st.markdown('<h2 class="page-title">中国网文类型以及各类型书目</h2>', unsafe_allow_html=True)
    st.button("返回首页", on_click=go_to_home, className="nav-btn")
    st.info('因为中国的每种类型小说都太多了，在此我仅举一些例子，或经典，或有代表性。如果你想要了解更多，或者有更详细的需求，欢迎去使用“书荒查询”')
...
def render_category_detail():
    ...
    st.markdown(f'<h2 class="page-title">{category}类网文小说</h2>', unsafe_allow_html=True)
    st.write(f"本类别共收录 {len(default_novel_data[category])} 本经典小说")
    if st.button("返回类型列表", className="nav-btn"):
        go_to_categories()
    ...
    for i, novel in enumerate(current_novels, start=start_idx+1):
        with cols[i % 2]:
            st.markdown('<div class="novel-card">', unsafe_allow_html=True)
            status_label = "连载中" if novel['status'] == "连载中" else "已完结"
            st.subheader(f"{i}. 《{novel['title']}》")
            st.write(f"作者：{novel['author']} | 状态：{status_label}")
...
def render_book_search():
    st.markdown('<h2 class="page-title">书荒查询</h2>', unsafe_allow_html=True)
    st.button("返回首页", on_click=go_to_home, className="nav-btn")
    ...
    with st.form("book_recommendation_form"):
        ...
        submitted = st.form_submit_button("获取推荐", className="nav-btn")
...
def render_character_dialog():
    st.markdown('<h2 class="page-title">与角色对话</h2>', unsafe_allow_html=True)
    st.button("返回首页", on_click=go_to_home, className="nav-btn")
    ...
    if not st.session_state.selected_character:
        ...
        if st.button("返回选择其他小说", use_container_width=True, className="nav-btn"):
            st.session_state.selected_novel = None
            st.session_state.selected_character = None
        st.markdown('</div>', unsafe_allow_html=True)
        return
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("选择其他角色", use_container_width=True, className="nav-btn"):
            st.session_state.selected_character = None
    with col2:
        if st.button("选择其他小说", use_container_width=True, className="nav-btn"):
            st.session_state.selected_novel = None
            st.session_state.selected_character = None
    st.markdown('</div>', unsafe_allow_html=True)
...
def render_story_mode():
    st.markdown('<h2 class="page-title">穿书play</h2>', unsafe_allow_html=True)
    st.button("返回首页", on_click=go_to_home, className="nav-btn")
    ...
    if not st.session_state.selected_theme:
        ...
        for theme in default_themes_data:
            with st.expander(f"{theme['name']}"):
                ...
                if st.button(f"选择【{theme['name']}】", use_container_width=True, className="nav-btn"):
                    select_theme(theme)
        st.markdown('</div>', unsafe_allow_html=True)
        return
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("重新开始这个主题", use_container_width=True, className="nav-btn"):
            select_theme(st.session_state.selected_theme)
    with col2:
        if st.button("选择其他主题", use_container_width=True, className="nav-btn"):
            st.session_state.selected_theme = None
            st.session_state.story_progress = []
            st.session_state.current_story_step = None

#################################################################################################################################################################################################################################
# 渲染开发者喜欢的小说（核心修改：按钮→图片跳转）
default_my_love_novel_data = {
    "我不是戏神","十日终焉","诡秘之主","神秘复苏","我在精神病院学斩神","诸神愚戏","斗破苍穹",
    "开局地摊卖大力","我一只史莱姆吊打巨龙很合理吧","遮天","雪中悍刀行","盗墓笔记","夜的命名术","道诡异仙"
}
novel_names = list(default_my_love_novel_data)  # 小说名称列表（与封面图顺序对应）

def render_my_love_novel():
    st.markdown('<h2 class="page-title">开发者喜欢的小说！（加上一点介绍）</h2>', unsafe_allow_html=True)
    st.button("返回首页", on_click=go_to_home, className="nav-btn")
    
    # 小说图片网格（替换原按钮）
    cols = st.columns(4)  # 每行4本小说
    novel_jump_funcs = [
        go_to_novel1, go_to_novel2, go_to_novel3, go_to_novel4,
        go_to_novel5, go_to_novel6, go_to_novel7, go_to_novel8,
        go_to_novel9, go_to_novel10, go_to_novel11, go_to_novel12,
        go_to_novel13, go_to_novel14
    ]  # 跳转函数列表（与小说顺序对应）
    ...
    for i in range(len(novel_names)):
        with cols[i % 4]:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            # 小说封面图 + 隐藏触发按钮
            st.image(fav_novel_covers[i], use_column_width=True, caption=novel_names[i], className="img-btn")
            if st.button(f"novel_{i}_trigger", key=f"novel_{i}_trigger", className="hidden-btn"):
                novel_jump_funcs[i]()
            # 点击图片触发跳转
            st.markdown(f"""
            <script>
            document.querySelector('[alt="{novel_names[i]}"]').onclick = function() {{
                document.querySelectorAll('[data-testid="stButton"]')[{i+1}].click();
            }}
            </script>
            """, unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
...
# 14本小说详情页（仅添加样式，逻辑不变）
def render_novel1():
    st.markdown('<h2 class="page-title">我不是戏神</h2>', unsafe_allow_html=True)
    st.button('返回作者喜欢的书目', on_click=go_to_my_love_novel, className="nav-btn")
...
def render_novel2():
    st.markdown('<h2 class="page-title">十日终焉</h2>', unsafe_allow_html=True)
    st.button('返回作者喜欢的书目', on_click=go_to_my_love_novel, className="nav-btn")
    photo_path='photo/srzy.webp'
    st.image(photo_path)
    st.markdown('<div class="card">', unsafe_allow_html=True)
...
def render_novel3():
    st.markdown('<h2 class="page-title">诡秘之主</h2>', unsafe_allow_html=True)
    st.button('返回作者喜欢的书目', on_click=go_to_my_love_novel, className="nav-btn")
...
def render_novel5():
    ...
    photo_path='photo/dgyxOIP-C.webp'
    st.image(photo_path)
