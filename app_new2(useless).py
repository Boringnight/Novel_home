...
<style>
    ...
    * {
    ...
    body {
        ...
        background-image: 
            radial-gradient(#e0e7ff 0.5px, transparent 0.5px),
            radial-gradient(#e0e7ff 0.5px, #f5f7fa 0.5px);
...
def render_home():
    ...
    st.markdown('<p style="text-align: center; font-size: 1.2rem; color: #4a5568;">—— 清华大学水木书院秦奕扬</p>', unsafe_allow_html=True)
    st.markdown('<p style="text-align: center; margin: 20px 0; font-size: 1.1rem; color: #4a5568;">探索中国网络文学的魅力，发现属于你的精彩故事</p>', unsafe_allow_html=True)
    ...
    with col1:
        with st.container():
            ...
            st.write("了解中国网络文学的起源与发展历程，探索网文文化的演变")
    ...
    with st.container():
        st.subheader("开发者推荐")
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.info("作为一个资深小说迷，我精选了一些优质作品推荐给大家：")
        ...
        recommended = [
            ...
            ("诸神愚戏", go_to_novel6),
            ("斗破苍穹", go_to_novel7),
            ("开局地摊卖大力", go_to_novel8),
            ("夜的命名术", go_to_novel9),
            ("遮天", go_to_novel10)
        ]
        
        for i, (name, func) in enumerate(recommended):
            with cols[i % 5]:
    ...
    if valid_images:
        ...
        # 每行显示4张图片
        for i in range(0, len(bordered_images), 4):
            row_images = bordered_images[i:i+4]
            cols = st.columns(len(row_images))
            for col, img in zip(cols, row_images):
                with col:
                    st.image(img,use_container_width=True, caption=f"热门小说 {i+cols.index(col)+1}")
...
def render_history():
    ...
    with st.container():
        ...
        with st.expander("历史时期导航", expanded=True):
            periods = ["起源阶段", "发展阶段", "繁荣阶段", "多元化阶段"]
            for period in periods:
                st.markdown(f'<a href="#{period}" style="text-decoration: none; color: #3182ce;">{period}</a>', unsafe_allow_html=True)
        
        # 优化文本显示格式
        st.markdown(history_text.replace(
            "一、", "## 一、").replace(
            "二、", "## 二、").replace(
            "三、", "## 三、").replace(
            "四、", "## 四、"
...
def render_categories():
    ...
    with st.container():
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.info('中国网络文学类型丰富多样，以下为主要分类，点击可查看代表作品：')
        ...
        for i, category in enumerate(categories):
            with cols[i % 3]:
                # 添加分类标签样式
                st.markdown(f'<div class="tag tag-primary">{category}</div>', unsafe_allow_html=True)
                if st.button(f"查看 {len(default_novel_data[category])} 本作品", use_container_width=True):
...
def render_category_detail():
    ...
    with col_back:
        if st.button("返回类型列表"):
            go_to_categories()
    with col_info:
        st.success(f"本类别共收录 {len(default_novel_data[category])} 本经典小说")
    ...
    with st.container():
        ...
        for i, novel in enumerate(current_novels, start=start_idx+1):
            ...
            status_color = "#e53e3e" if status_label == "连载中" else "#38a169"
            
            with st.expander(f"{i}. 《{novel['title']}》- {novel['author']} <span style='color:{status_color};'>[{status_label}]</span>", unsafe_allow_html=True):
                # 显示标签
                for tag in novel['tags']:
                    st.markdown(f'<span class="tag tag-primary">{tag}</span>', unsafe_allow_html=True)
...
def render_book_search():
    ...
    with st.container():
        ...
        with st.expander("查看推荐示例", expanded=False):
            st.write("例如：")
            st.code("我喜欢玄幻类小说，特别是有修炼体系和冒险元素的，类似《斗破苍穹》这样的作品")
            st.code("想找一些克苏鲁风格的网络小说，带有悬疑和恐怖元素")
        ...
        with st.form("book_recommendation_form"):
            ...
            if submitted and user_input:
                with st.spinner("AI正在为您分析并推荐书籍..."):
                    ...
                    st.success("推荐结果：")
                    # 优化推荐结果显示
                    st.info(recommendation.replace("#", "\n\n"))
            elif submitted:
...
def render_character_dialog():
    ...
    with st.container():
        ...
        # 显示对话历史，优化样式
        st.markdown('<div style="background-color: #f7fafc; padding: 1rem; border-radius: 8px; margin-bottom: 1rem; max-height: 400px; overflow-y: auto;">', unsafe_allow_html=True)
        for message in st.session_state.dialog_history:
            if message["role"] == "user":
                st.markdown(f'<div style="text-align: right; margin: 0.5rem 0;"><span style="background-color: #3182ce; color: white; padding: 0.5rem 1rem; border-radius: 18px; display: inline-block;">{message["content"]}</span></div>', unsafe_allow_html=True)
            else:
                st.markdown(f'<div style="text-align: left; margin: 0.5rem 0;"><span style="background-color: #e2e8f0; color: #2d3748; padding: 0.5rem 1rem; border-radius: 18px; display: inline-block;">{message["content"]}</span></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
def render_story_mode():
    ...
    with st.container():
        ...
        if not st.session_state.selected_theme:
            ...
            # 使用网格布局展示主题
            cols = st.columns(2)
            for i, theme in enumerate(default_themes_data):
                with cols[i % 2]:
        ...
        # 显示故事进展，优化滚动区域
        st.write("### 故事进展：")
        st.markdown('<div style="background-color: #f7fafc; padding: 1rem; border-radius: 8px; margin-bottom: 1rem; max-height: 300px; overflow-y: auto;">', unsafe_allow_html=True)
...
default_my_love_novel_data = {
    "我不是戏神","十日终焉","诡秘之主","神秘复苏","我在精神病院学斩神",
    "诸神愚戏","斗破苍穹","开局地摊卖大力","我一只史莱姆吊打巨龙很合理吧",
    "遮天","雪中悍刀行","盗墓笔记","夜的命名术","道诡异仙"
}

def render_my_love_novel():
    st.markdown('<h1 class="main-header fade-in">开发者喜欢的小说</h1>', unsafe_allow_html=True)
    if st.button("返回首页", use_container_width=True):
        go_to_home()
    
    with st.container():
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.info("这些是我精心挑选的优质网络小说，每本都有其独特之处：")
        ...
        novels = [
            ...
            ("雪中悍刀行", go_to_novel11),
            ("盗墓笔记", go_to_novel12),
            ("我在精神病院学斩神", go_to_novel13),
            ("我一只史莱姆吊打巨龙很合理吧", go_to_novel14)
        ]
        
        for i, (name, func) in enumerate(novels):
            with cols[i % 4]:
                if st.button(name, use_container_width=True):
                    func()
...
def render_writing():
    ...
    with st.container():
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.write('请输入小说的基础设定、关键词或情节构思，AI将协助你生成小说章节：')
        
        # 提供小说创作模板
        with st.expander("查看创作提示模板", expanded=False):
            st.write("可以参考以下格式输入：")
            st.code("类型：玄幻\n主角：李逍遥，一个普通少年\n世界观：存在修仙者的世界\n情节：主角意外获得上古传承，开始修炼之路\n风格：轻松幽默，带有热血战斗")
        
        # 多字段输入表单
        with st.form("novel_writing_form"):
            col1, col2 = st.columns(2)
            with col1:
                novel_type = st.text_input("小说类型：", placeholder="如：玄幻、科幻、悬疑等")
                main_character = st.text_input("主角设定：", placeholder="姓名、性格、背景等")
            with col2:
                world_view = st.text_input("世界观：", placeholder="故事发生的世界背景")
                style = st.text_input("风格：", placeholder="如：轻松、严肃、恐怖等")
            
            plot = st.text_area(
                "情节构思：",
                placeholder="描述你想写的主要情节或关键事件...",
                height=100
            )
            
            # 合并所有信息为创作需求
            user_demand = f"类型：{novel_type}\n主角：{main_character}\n世界观：{world_view}\n风格：{style}\n情节：{plot}"
